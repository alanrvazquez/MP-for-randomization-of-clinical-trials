...
all_metrics <- read.csv("Study_Tuning_Parameters/metrics_pembro_rho_CARO.csv")
...
pdf(file = "Study_Tuning_Parameters/Pembro_Rho.pdf", width = 11, height = 8) # defaults to 7 x 7 inches
par(mfrow = c(1, 2))
...
all_metrics <- read.csv("Study_Tuning_Parameters/metrics_spasms_rho_CARO.csv")
all_metrics <- all_metrics[,-1]
...
pdf(file = "Study_Tuning_Parameters/Spasms_Rho.pdf", width = 11, height = 8) # defaults to 7 x 7 inches
par(mfrow = c(1, 2))
boxplot(Energy~Rho, data = all_metrics, 
        ylab = 'Energy', ylim = c(0, 1),
        cex.lab = lab.font.size, cex.axis = ax.font.size)
