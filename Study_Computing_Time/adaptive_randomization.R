...
kernel.randomization <- function(x, Z, n.zero = 8, bc.prob = 0.8,
  ...
  # Set object to record computing time.
  CPU.time <- rep(NA, times = n - n.zero)
  ...
  for (i in (n.zero+1):n){
    ...
    CPU.time[i - n.zero] <- proc.time()[3] - start.time[3]
  ...
  group.assignment$MeanTime <- mean(CPU.time, na.rm = T)
