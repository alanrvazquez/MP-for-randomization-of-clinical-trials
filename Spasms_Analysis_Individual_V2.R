...
load("designs/SpasmsDesigns_Individual_v2.RData")
...
my.col.names <- c("Method", "Balance", "M.Age", "M.ISFreq", 
                  "SD.Age", "SD.ISFreq",  "Energy", "M.CG", "SD.CG")
...
aggregate.metrics <- aggregate(all_metrics[,-1], by = list(all_metrics$Method), 
                               FUN = mean)
data.p <- aggregate.metrics[, c(2, 7, 8)] # aggregate.metrics[,-c(1, 11)]
